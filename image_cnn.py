...
# 数据加载
train_dataset = SpectrogramDataset(r'D:\aa_my_learning\my_learining\项目1\小波同步压缩变换时频\train_img', train_transform)
val_dataset = SpectrogramDataset(r'D:\aa_my_learning\my_learining\项目1\小波同步压缩变换时频\valid_img', test_transform)
test_dataset = SpectrogramDataset(r'D:\aa_my_learning\my_learining\项目1\小波同步压缩变换时频\test_img', test_transform)
...
# 模型初始化
model = CNN(num_classes=6, base_channels=34, extra_conv_layers=1,
            use_res=True, dropout_rate=0.355956,kernel_size=3).to(device)
optimizer = optim.AdamW(model.parameters(), lr=0.000037, weight_decay=  0.000007)
...
test_loader = DataLoader(
    SpectrogramDataset(r'D:\aa_my_learning\my_learining\项目1\小波同步压缩变换时频\test_img', test_transform),
    batch_size=32,
    shuffle=False
)
